...
export default {
  ...
  methods: {
      ...
      this.$refs.accasstypeForm.validate((valid) => {
        if(valid){
          if(type === 'form' || type === 'table-form'){
            ...
            var url = ylsBusi+'iface/accasstype/create';
            if(type === 'table-form'){
              url = ylsBusi+'iface/accasstype/update';
            }
            this.$http({
              url: url,
              method: 'POST',
    ...
    deleteConfirmClick() {
      this.$http({
        url: ylsBusi+"/iface/accasstype/deleteById",
    ...
    //后台请求
    request() {
      var url = ylsBusi+"iface/accasstype/page";
      var data = {
        orderList: [
          {
            direction: "desc",
